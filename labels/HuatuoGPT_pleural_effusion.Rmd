...
output_file <- "E:/xqjy_results.txt"  # 合并输出结果文件（TXT格式）
log_file <- "E:/xqjy_analysis.log"  # 详细日志文件
max_retry <- 3  # 最大重试次数
...
# --------------------------
# 改进的结果解析函数（判断是否存在胸腔积液）
# --------------------------
parse_model_output <- function(output, report_text) {
    output_lower <- tolower(output)
    
    # 1. 检查标准格式输出（存在胸腔积液: 是/否）
    if (grepl("存在胸腔积液:\\s*是", output, ignore.case = TRUE)) {
        return(list(has_effusion = TRUE, evidence = "模型明确判断存在"))
    }
    if (grepl("存在胸腔积液:\\s*否", output, ignore.case = TRUE)) {
        return(list(has_effusion = FALSE, evidence = "模型明确判断不存在"))
    }
    
    # 2. 检查其他肯定表述（如"有胸腔积液"）
    positive_terms <- c("存在胸腔积液", "有胸腔积液", "可见胸腔积液", 
                       "右侧胸腔积液", "左侧胸腔积液", "双侧胸腔积液")
    if (any(sapply(positive_terms, function(term) grepl(term, output, ignore.case = TRUE)))) {
        return(list(has_effusion = TRUE, evidence = "模型表述中包含肯定词汇"))
    }
    
    # 3. 检查否定表述（如"未见胸腔积液"）
    negative_terms <- c("未见胸腔积液", "无胸腔积液", "未发现胸腔积液", 
                       "胸腔未见异常", "胸腔清晰")
    if (any(sapply(negative_terms, function(term) grepl(term, output, ignore.case = TRUE)))) {
        return(list(has_effusion = FALSE, evidence = "模型表述中包含否定词汇"))
    }
    
    # 4. 直接检查报告原文是否包含关键词（最终兜底判断）
    if (any(grepl(paste(effusion_keywords, collapse = "|"), report_text, ignore.case = TRUE))) {
        return(list(has_effusion = TRUE, evidence = "报告原文包含胸腔积液相关关键词"))
    }
    
    # 5. 所有条件都不满足时，默认判断为不存在
    return(list(has_effusion = FALSE, evidence = "未发现明确证据"))
}
    ...
    for (attempt in 1:max_retry) {
        tryCatch({
            ...
            if (length(output) > 0) {
                ...
                return(list(
                    success = TRUE,
                    has_effusion = parsed_result$has_effusion,
...
main <- function() {
    ...
    for (i in 1:total_files) {
        ...
        tryCatch({
            ...
            result_value <- ifelse(analysis_result$has_effusion, 1, 0)
            ...
            if (analysis_result$success) {
                ...
                if (analysis_result$has_effusion) {
                    effusion_count <- effusion_count + 1
                }
    ...
    cat(paste0("存在胸腔积液的报告数: ", effusion_count, " 份\n"))
    ...
    log_message(paste0("存在胸腔积液的报告数: ", effusion_count, " 份"))
