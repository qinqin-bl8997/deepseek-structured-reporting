...
# 定义调用DeepSeek API的函数（判断单份报告是否存在水肿）
check_edema <- function(report_content, api_key, model = "deepseek-chat") {
  api_url <- "https://api.deepseek.com/v1/chat/completions"
  
  # 构建提示词，明确判断目标为edema（水肿）
  prompt <- paste0(
    "请分析以下胸片报告内容，判断是否存在水肿（edema）的诊断。\n",
  ...
  payload <- list(
    model = model,
    messages = list(
      list(role = "system", content = "你是放射科医生，擅长识别肺部水肿（edema）的影像学表现。"),
...
analyze_reports <- function(file_path, api_key) {
  ...
  for (part in report_parts) {
    ...
    if (content != "") {
      # 调用API判断水肿
      result <- check_edema(content, api_key)
