...
output_file <- "E:/consolidation_results.txt"  # 合并输出结果文件（TXT格式）
log_file <- "E:/consolidation_analysis.log"  # 详细日志文件
max_retry <- 3  # 最大重试次数
...
# 实变相关关键词（用于辅助判断）
consolidation_keywords <- c(
    "实变", "肺实变", "肺内实变", "实变影", 
    "密度增高影", "斑片影", "片状影", "絮状影",
    "consolidation", "consolidate", "infiltrate", 
    "patchy opacity", "airspace consolidation", "肺泡实变",
    "肺段实变", "肺叶实变", "炎性实变"
)
...
parse_model_output <- function(output, report_text) {
    ...
    # 1. 检查标准格式输出（存在实变: 是/否）
    if (grepl("存在实变:\\s*是", output, ignore.case = TRUE)) {
        return(list(has_consolidation = TRUE, evidence = "模型明确判断存在"))
    }
    if (grepl("存在实变:\\s*否", output, ignore.case = TRUE)) {
        return(list(has_consolidation = FALSE, evidence = "模型明确判断不存在"))
    }
    
    # 2. 检查其他肯定表述（如"有实变"）
    positive_terms <- c("存在实变", "有实变", "可见实变", 
                       "肺实变", "实变影", "斑片影",
                       "片状实变", "肺泡实变", "段性实变")
    if (any(sapply(positive_terms, function(term) grepl(term, output, ignore.case = TRUE)))) {
        return(list(has_consolidation = TRUE, evidence = "模型表述中包含肯定词汇"))
    }
    
    # 3. 检查否定表述（如"未见实变"）
    negative_terms <- c("未见实变", "无实变", "未发现实变", 
                       "肺野清晰", "无明显实变", "无斑片影")
    if (any(sapply(negative_terms, function(term) grepl(term, output, ignore.case = TRUE)))) {
        return(list(has_consolidation = FALSE, evidence = "模型表述中包含否定词汇"))
    }
    
    # 4. 直接检查报告原文是否包含关键词（最终兜底判断）
    if (any(grepl(paste(consolidation_keywords, collapse = "|"), report_text, ignore.case = TRUE))) {
        return(list(has_consolidation = TRUE, evidence = "报告原文包含实变相关关键词"))
    }
    
    # 5. 所有条件都不满足时，默认判断为不存在
    return(list(has_consolidation = FALSE, evidence = "未发现明确证据"))
}
...
analyze_single_report <- function(report_text, report_filename) {
    # 创建提示词（明确要求模型判断）
    prompt <- paste0(
        "你是专业的放射科医生，请分析以下胸片报告是否存在实变（包括肺内实变、斑片影等）。\n",
        "只需根据报告内容回答，不要添加额外信息。\n",
        "输出格式必须为：\n",
        "存在实变: 是\n",  # 或 "存在实变: 否"
        "证据: （引用报告中支持判断的原文）\n\n",
    ...
    for (attempt in 1:max_retry) {
        tryCatch({
            ...
            if (length(output) > 0) {
                ...
                return(list(
                    success = TRUE,
                    has_consolidation = parsed_result$has_consolidation,
...
main <- function() {
    ...
    success_count <- 0
    consolidation_count <- 0  # 存在实变的报告数
    ...
    for (i in 1:total_files) {
        ...
        tryCatch({
            ...
            result_value <- ifelse(analysis_result$has_consolidation, 1, 0)
            ...
            if (analysis_result$success) {
                ...
                if (analysis_result$has_consolidation) {
    ...
    cat(paste0("存在实变的报告数: ", consolidation_count, " 份\n"))
    ...
    log_message(paste0("存在实变的报告数: ", consolidation_count, " 份"))
