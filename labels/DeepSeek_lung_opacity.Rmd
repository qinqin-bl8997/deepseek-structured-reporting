...
# 定义调用DeepSeek API的函数（判断单份报告是否存在肺部阴影）
check_lung_opacity <- function(report_content, api_key, model = "deepseek-chat") {
  api_url <- "https://api.deepseek.com/v1/chat/completions"
  
  # 构建提示词，明确判断目标为lung opacity（肺部阴影）
  prompt <- paste0(
    "请分析以下胸片报告内容，判断是否存在肺部阴影（lung opacity）的诊断。\n",
  ...
  payload <- list(
    model = model,
    messages = list(
      list(role = "system", content = "你是放射科医生，擅长识别肺部阴影（lung opacity）的影像学表现。"),
...
analyze_reports <- function(file_path, api_key) {
  ...
  for (part in report_parts) {
    ...
    if (content != "") {
      # 调用API判断肺部阴影
      result <- check_lung_opacity(content, api_key)
