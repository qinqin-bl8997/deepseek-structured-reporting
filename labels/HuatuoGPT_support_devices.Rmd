...
output_file <- "E:/support_devices_results.txt"  # 合并输出结果文件（TXT格式）
log_file <- "E:/support_devices_analysis.log"  # 详细日志文件
max_retry <- 3  # 最大重试次数
...
# 患者支持设备相关关键词（用于辅助判断）
support_devices_keywords <- c(
    "呼吸机", "气管插管", "鼻导管", "面罩", "ECMO", "引流管", 
    "胃管", "尿管", "中心静脉导管", "PICC", "IABP", "支架",
    "pacemaker", "ventilator", "tracheal tube", "nasal cannula",
    "drainage tube", "gastric tube", "urinary catheter", "stent"
)
...
# --------------------------
# 改进的结果解析函数（判断是否存在患者支持设备）
# --------------------------
parse_model_output <- function(output, report_text) {
    output_lower <- tolower(output)
    
    # 1. 检查标准格式输出（存在患者支持设备: 是/否）
    if (grepl("存在患者支持设备:\\s*是", output, ignore.case = TRUE)) {
        return(list(has_support_devices = TRUE, evidence = "模型明确判断存在"))
    }
    if (grepl("存在患者支持设备:\\s*否", output, ignore.case = TRUE)) {
        return(list(has_support_devices = FALSE, evidence = "模型明确判断不存在"))
    }
    
    # 2. 检查其他肯定表述（如"有患者支持设备"）
    positive_terms <- c("存在患者支持设备", "有患者支持设备", "可见患者支持设备", 
                       "呼吸机", "气管插管", "鼻导管", "面罩", "引流管")
    if (any(sapply(positive_terms, function(term) grepl(term, output, ignore.case = TRUE)))) {
        return(list(has_support_devices = TRUE, evidence = "模型表述中包含肯定词汇"))
    }
    
    # 3. 检查否定表述（如"未见患者支持设备"）
    negative_terms <- c("未见患者支持设备", "无患者支持设备", "未发现患者支持设备", 
                       "无辅助设备", "未使用支持设备")
    if (any(sapply(negative_terms, function(term) grepl(term, output, ignore.case = TRUE)))) {
        return(list(has_support_devices = FALSE, evidence = "模型表述中包含否定词汇"))
    }
    
    # 4. 直接检查报告原文是否包含关键词（最终兜底判断）
    if (any(grepl(paste(support_devices_keywords, collapse = "|"), report_text, ignore.case = TRUE))) {
        return(list(has_support_devices = TRUE, evidence = "报告原文包含患者支持设备相关关键词"))
    }
    
    # 5. 所有条件都不满足时，默认判断为不存在
    return(list(has_support_devices = FALSE, evidence = "未发现明确证据"))
}
...
analyze_single_report <- function(report_text, report_filename) {
    # 创建提示词（明确要求模型判断）
    prompt <- paste0(
        "你是专业的重症监护医生，请分析以下胸片报告中是否存在患者支持设备（如呼吸机、气管插管、引流管等）。\n",
        "只需根据报告内容回答，不要添加额外信息。\n",
        "输出格式必须为：\n",
        "存在患者支持设备: 是\n",  # 或 "存在患者支持设备: 否"
        "证据: （引用报告中支持判断的原文）\n\n",
    ...
    for (attempt in 1:max_retry) {
        tryCatch({
            ...
            if (length(output) > 0) {
                ...
                return(list(
                    success = TRUE,
                    has_support_devices = parsed_result$has_support_devices,
...
main <- function() {
    ...
    cat("文件名,结果(1=有患者支持设备,0=无患者支持设备)\n", file = output_file, useBytes = TRUE)
    ...
    success_count <- 0
    support_devices_count <- 0  # 存在患者支持设备的报告数
    ...
    for (i in 1:total_files) {
        ...
        tryCatch({
            ...
            result_value <- ifelse(analysis_result$has_support_devices, 1, 0)
            ...
            if (analysis_result$success) {
                ...
                if (analysis_result$has_support_devices) {
                    support_devices_count <- support_devices_count + 1
                }
    ...
    cat(paste0("存在患者支持设备的报告数: ", support_devices_count, " 份\n"))
    ...
    log_message(paste0("存在患者支持设备的报告数: ", support_devices_count, " 份"))
