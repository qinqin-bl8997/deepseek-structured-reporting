...
# 定义调用DeepSeek API的函数（判断单份报告是否存在实变）
check_consolidation <- function(report_content, api_key, model = "deepseek-chat") {
  api_url <- "https://api.deepseek.com/v1/chat/completions"
  
  # 构建提示词，明确判断目标为consolidation（实变）
  prompt <- paste0(
    "请分析以下胸片报告内容，判断是否存在实变（consolidation）的诊断。\n",
  ...
  payload <- list(
    model = model,
    messages = list(
      list(role = "system", content = "你是放射科医生，擅长识别肺部实变（consolidation）的影像学表现。"),
...
analyze_reports <- function(file_path, api_key) {
  ...
  for (part in report_parts) {
    ...
    if (content != "") {
      # 调用API判断实变
      result <- check_consolidation(content, api_key)
