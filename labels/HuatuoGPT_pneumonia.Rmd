...
output_file <- "E:/pneumonia_results.txt"  # 合并输出结果文件（TXT格式）
log_file <- "E:/pneumonia_analysis.log"  # 详细日志文件
max_retry <- 3  # 最大重试次数
...
# 肺炎相关关键词（用于辅助判断）
pneumonia_keywords <- c(
    "肺炎", "细菌性肺炎", "病毒性肺炎", "间质性肺炎",
    "支气管肺炎", "大叶性肺炎", "肺炎实变", "炎症浸润",
    "斑片影", "片状模糊影", "磨玻璃影", "实变影",
    "pneumonia", "bacterial pneumonia", "viral pneumonia",
    "infiltrate", "consolidation", "alveolar opacity"
)
...
parse_model_output <- function(output, report_text) {
    ...
    # 1. 检查标准格式输出（存在肺炎: 是/否）
    if (grepl("存在肺炎:\\s*是", output, ignore.case = TRUE)) {
        return(list(has_pneumonia = TRUE, evidence = "模型明确判断存在"))
    }
    if (grepl("存在肺炎:\\s*否", output, ignore.case = TRUE)) {
        return(list(has_pneumonia = FALSE, evidence = "模型明确判断不存在"))
    }
    
    # 2. 检查其他肯定表述（如"有肺炎"）
    positive_terms <- c("存在肺炎", "有肺炎", "可见肺炎", 
                       "细菌性肺炎", "病毒性肺炎", "支气管肺炎",
                       "大叶性肺炎", "肺炎实变", "炎症浸润影")
    if (any(sapply(positive_terms, function(term) grepl(term, output, ignore.case = TRUE)))) {
        return(list(has_pneumonia = TRUE, evidence = "模型表述中包含肯定词汇"))
    }
    
    # 3. 检查否定表述（如"未见肺炎"）
    negative_terms <- c("未见肺炎", "无肺炎", "未发现肺炎", 
                       "肺野清晰", "无明显炎症", "无斑片影")
    if (any(sapply(negative_terms, function(term) grepl(term, output, ignore.case = TRUE)))) {
        return(list(has_pneumonia = FALSE, evidence = "模型表述中包含否定词汇"))
    }
    
    # 4. 直接检查报告原文是否包含关键词（最终兜底判断）
    if (any(grepl(paste(pneumonia_keywords, collapse = "|"), report_text, ignore.case = TRUE))) {
        return(list(has_pneumonia = TRUE, evidence = "报告原文包含肺炎相关关键词"))
    }
    
    # 5. 所有条件都不满足时，默认判断为不存在
    return(list(has_pneumonia = FALSE, evidence = "未发现明确证据"))
}
...
analyze_single_report <- function(report_text, report_filename) {
    # 创建提示词（明确要求模型判断）
    prompt <- paste0(
        "你是专业的放射科医生，请分析以下胸片报告是否存在肺炎（包括各类肺炎及炎症浸润表现）。\n",
        "只需根据报告内容回答，不要添加额外信息。\n",
        "输出格式必须为：\n",
        "存在肺炎: 是\n",  # 或 "存在肺炎: 否"
        "证据: （引用报告中支持判断的原文）\n\n",
    ...
    for (attempt in 1:max_retry) {
        tryCatch({
            ...
            if (length(output) > 0) {
                ...
                return(list(
                    success = TRUE,
                    has_pneumonia = parsed_result$has_pneumonia,
...
main <- function() {
    ...
    for (i in 1:total_files) {
        ...
        tryCatch({
            ...
            result_value <- ifelse(analysis_result$has_pneumonia, 1, 0)
            ...
            if (analysis_result$success) {
                ...
                if (analysis_result$has_pneumonia) {
    ...
    cat(paste0("存在肺炎的报告数: ", pneumonia_count, " 份\n"))
    ...
    log_message(paste0("存在肺炎的报告数: ", pneumonia_count, " 份"))
