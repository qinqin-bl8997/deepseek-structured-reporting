...
output_file <- "E:/atelectasis_results.txt"  # 合并输出结果文件（TXT格式）
log_file <- "E:/atelectasis_analysis.log"  # 详细日志文件
max_retry <- 3  # 最大重试次数
...
# 肺不张相关关键词（用于辅助判断）
atelectasis_keywords <- c(
    "肺不张", "肺萎陷", "肺膨胀不全", "叶间裂移位", 
    "容积缩小", "密度增高", "atel", "atelectasis",
    "collapse", "consolidation", "肺叶不张", "段性不张"
)
...
parse_model_output <- function(output, report_text) {
    ...
    # 1. 检查标准格式输出（存在肺不张: 是/否）
    if (grepl("存在肺不张:\\s*是", output, ignore.case = TRUE)) {
        return(list(has_atelectasis = TRUE, evidence = "模型明确判断存在"))
    }
    if (grepl("存在肺不张:\\s*否", output, ignore.case = TRUE)) {
        return(list(has_atelectasis = FALSE, evidence = "模型明确判断不存在"))
    }
    
    # 2. 检查其他肯定表述（如"有肺不张"）
    positive_terms <- c("存在肺不张", "有肺不张", "可见肺不张", 
                       "右侧肺不张", "左侧肺不张", "肺叶不张")
    if (any(sapply(positive_terms, function(term) grepl(term, output, ignore.case = TRUE)))) {
        return(list(has_atelectasis = TRUE, evidence = "模型表述中包含肯定词汇"))
    }
    
    # 3. 检查否定表述（如"未见肺不张"）
    negative_terms <- c("未见肺不张", "无肺不张", "未发现肺不张", 
                       "肺野清晰", "肺野未见异常")
    if (any(sapply(negative_terms, function(term) grepl(term, output, ignore.case = TRUE)))) {
        return(list(has_atelectasis = FALSE, evidence = "模型表述中包含否定词汇"))
    }
    
    # 4. 直接检查报告原文是否包含关键词（最终兜底判断）
    if (any(grepl(paste(atelectasis_keywords, collapse = "|"), report_text, ignore.case = TRUE))) {
        return(list(has_atelectasis = TRUE, evidence = "报告原文包含肺不张相关关键词"))
    }
    
    # 5. 所有条件都不满足时，默认判断为不存在
    return(list(has_atelectasis = FALSE, evidence = "未发现明确证据"))
}
    ...
    for (attempt in 1:max_retry) {
        tryCatch({
            ...
            if (length(output) > 0) {
                ...
                return(list(
                    success = TRUE,
                    has_atelectasis = parsed_result$has_atelectasis,
...
main <- function() {
    ...
    for (i in 1:total_files) {
        ...
        tryCatch({
            ...
            result_value <- ifelse(analysis_result$has_atelectasis, 1, 0)
            ...
            if (analysis_result$success) {
                ...
                if (analysis_result$has_atelectasis) {
                    atelectasis_count <- atelectasis_count + 1
                }
    ...
    cat(paste0("存在肺不张的报告数: ", atelectasis_count, " 份\n"))
    ...
    log_message(paste0("存在肺不张的报告数: ", atelectasis_count, " 份"))
