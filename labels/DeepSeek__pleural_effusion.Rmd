...
check_pleural_effusion <- function(report_content, api_key, model = "deepseek-chat") {
  api_url <- "https://api.deepseek.com/v1/chat/completions"
  
  # 构建提示词
  prompt <- paste0(
    "请分析以下胸片报告内容，判断是否存在胸腔积液的诊断。\n",
  ...
  payload <- list(
    model = model,
    messages = list(
      list(role = "system", content = "你是放射科医生，擅长识别胸腔积液诊断。"),
...
analyze_reports <- function(file_path, api_key) {
  ...
  for (part in report_parts) {
    ...
    if (content != "") {
      # 直接调用API判断
      result <- check_pleural_effusion(content, api_key)
