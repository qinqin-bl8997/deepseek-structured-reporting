...
output_file <- "E:/lung_lesion_results.txt"  # 合并输出结果文件（TXT格式）
log_file <- "E:/lung_lesion_analysis.log"  # 详细日志文件
max_retry <- 3  # 最大重试次数
...
# 肺部病灶相关关键词（用于辅助判断）
lung_lesion_keywords <- c(
    "肺部病灶", "肺内病灶", "病灶", "占位性病变",
    "结节", "肿块", "团块", "斑片灶", 
    "条索灶", "钙化灶", "空洞", "阴影灶",
    "lesion", "pulmonary lesion", "nodule", "mass",
    "opacity", "infiltrate", "focus"
)
...
# --------------------------
# 改进的结果解析函数（判断是否存在肺部病灶）
# --------------------------
parse_model_output <- function(output, report_text) {
    output_lower <- tolower(output)
    
    # 1. 检查标准格式输出（存在肺部病灶: 是/否）
    if (grepl("存在肺部病灶:\\s*是", output, ignore.case = TRUE)) {
        return(list(has_lung_lesion = TRUE, evidence = "模型明确判断存在"))
    }
    if (grepl("存在肺部病灶:\\s*否", output, ignore.case = TRUE)) {
        return(list(has_lung_lesion = FALSE, evidence = "模型明确判断不存在"))
    }
    
    # 2. 检查其他肯定表述（如"有肺部病灶"）
    positive_terms <- c("存在肺部病灶", "有肺部病灶", "可见肺部病灶", 
                       "肺内病灶", "占位性病变", "结节", 
                       "肿块", "斑片灶", "条索灶")
    if (any(sapply(positive_terms, function(term) grepl(term, output, ignore.case = TRUE)))) {
        return(list(has_lung_lesion = TRUE, evidence = "模型表述中包含肯定词汇"))
    }
    
    # 3. 检查否定表述（如"未见肺部病灶"）
    negative_terms <- c("未见肺部病灶", "无肺部病灶", "未发现肺部病灶", 
                       "肺野清晰", "肺内未见异常", "无明显病灶")
    if (any(sapply(negative_terms, function(term) grepl(term, output, ignore.case = TRUE)))) {
        return(list(has_lung_lesion = FALSE, evidence = "模型表述中包含否定词汇"))
    }
    
    # 4. 直接检查报告原文是否包含关键词（最终兜底判断）
    if (any(grepl(paste(lung_lesion_keywords, collapse = "|"), report_text, ignore.case = TRUE))) {
        return(list(has_lung_lesion = TRUE, evidence = "报告原文包含肺部病灶相关关键词"))
    }
    
    # 5. 所有条件都不满足时，默认判断为不存在
    return(list(has_lung_lesion = FALSE, evidence = "未发现明确证据"))
}
...
analyze_single_report <- function(report_text, report_filename) {
    # 创建提示词（明确要求模型判断）
    prompt <- paste0(
        "你是专业的放射科医生，请分析以下胸片报告是否存在肺部病灶（包括结节、肿块、斑片灶等）。\n",
        "只需根据报告内容回答，不要添加额外信息。\n",
        "输出格式必须为：\n",
        "存在肺部病灶: 是\n",  # 或 "存在肺部病灶: 否"
        "证据: （引用报告中支持判断的原文）\n\n",
    ...
    for (attempt in 1:max_retry) {
        tryCatch({
            ...
            if (length(output) > 0) {
                ...
                return(list(
                    success = TRUE,
                    has_lung_lesion = parsed_result$has_lung_lesion,
...
main <- function() {
    ...
    success_count <- 0
    lung_lesion_count <- 0  # 存在肺部病灶的报告数
    ...
    for (i in 1:total_files) {
        ...
        tryCatch({
            ...
            result_value <- ifelse(analysis_result$has_lung_lesion, 1, 0)
            ...
            if (analysis_result$success) {
                ...
                if (analysis_result$has_lung_lesion) {
                    lung_lesion_count <- lung_lesion_count + 1
                }
    ...
    cat(paste0("存在肺部病灶的报告数: ", lung_lesion_count, " 份\n"))
    ...
    log_message(paste0("存在肺部病灶的报告数: ", lung_lesion_count, " 份"))
