...
output_file <- "E:/fracture_results.txt"  # 合并输出结果文件（TXT格式）
log_file <- "E:/fracture_analysis.log"  # 详细日志文件
max_retry <- 3  # 最大重试次数
...
fracture_keywords <- c(
    "骨折", "骨断裂", "骨皮质不连续", "骨折线", 
    "粉碎性骨折", "线性骨折", "移位性骨折",
    "fracture", "bone fracture", "cortical discontinuity",
    "fracture line", "comminuted fracture", "displaced fracture"
)
...
parse_model_output <- function(output, report_text) {
    ...
    # 1. 检查标准格式输出（存在骨折: 是/否）
    if (grepl("存在骨折:\\s*是", output, ignore.case = TRUE)) {
        return(list(has_fracture = TRUE, evidence = "模型明确判断存在"))
    }
    if (grepl("存在骨折:\\s*否", output, ignore.case = TRUE)) {
        return(list(has_fracture = FALSE, evidence = "模型明确判断不存在"))
    }
    
    # 2. 检查其他肯定表述（如"有骨折"）
    positive_terms <- c("存在骨折", "有骨折", "可见骨折", 
                       "骨皮质不连续", "骨折线", "粉碎性骨折",
                       "线性骨折", "移位性骨折")
    if (any(sapply(positive_terms, function(term) grepl(term, output, ignore.case = TRUE)))) {
        return(list(has_fracture = TRUE, evidence = "模型表述中包含肯定词汇"))
    }
    
    # 3. 检查否定表述（如"未见骨折"）
    negative_terms <- c("未见骨折", "无骨折", "未发现骨折", 
                       "骨皮质连续", "无骨折线", "骨骼未见异常")
    if (any(sapply(negative_terms, function(term) grepl(term, output, ignore.case = TRUE)))) {
        return(list(has_fracture = FALSE, evidence = "模型表述中包含否定词汇"))
    }
    
    # 4. 直接检查报告原文是否包含关键词（最终兜底判断）
    if (any(grepl(paste(fracture_keywords, collapse = "|"), report_text, ignore.case = TRUE))) {
        return(list(has_fracture = TRUE, evidence = "报告原文包含骨折相关关键词"))
    }
    
    # 5. 所有条件都不满足时，默认判断为不存在
    return(list(has_fracture = FALSE, evidence = "未发现明确证据"))
}
    ...
    for (attempt in 1:max_retry) {
        tryCatch({
            ...
            if (length(output) > 0) {
                ...
                return(list(
                    success = TRUE,
                    has_fracture = parsed_result$has_fracture,
...
main <- function() {
    ...
    for (i in 1:total_files) {
        ...
        tryCatch({
            ...
            result_value <- ifelse(analysis_result$has_fracture, 1, 0)
            ...
            if (analysis_result$success) {
                ...
                if (analysis_result$has_fracture) {
    ...
    cat(paste0("存在骨折的报告数: ", fracture_count, " 份\n"))
    ...
    log_message(paste0("存在骨折的报告数: ", fracture_count, " 份"))
