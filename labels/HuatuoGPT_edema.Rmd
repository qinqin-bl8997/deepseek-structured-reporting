...
output_file <- "E:/edema_results.txt"  # 合并输出结果文件（TXT格式）
log_file <- "E:/edema_analysis.log"  # 详细日志文件
max_retry <- 3  # 最大重试次数
...
edema_keywords <- c(
    "水肿", "肺水肿", "间质性水肿", "肺泡性水肿", 
    "肺淤血", "蝶翼征", "Kerley B线", "胸腔积液",
    "心源性肺水肿", "血管充血", "水肿带", 
    "edema", "pulmonary edema", "congestion", "effusion"
)
...
parse_model_output <- function(output, report_text) {
    ...
    # 1. 检查标准格式输出（存在水肿: 是/否）
    if (grepl("存在水肿:\\s*是", output, ignore.case = TRUE)) {
        return(list(has_edema = TRUE, evidence = "模型明确判断存在"))
    }
    if (grepl("存在水肿:\\s*否", output, ignore.case = TRUE)) {
        return(list(has_edema = FALSE, evidence = "模型明确判断不存在"))
    }
    
    # 2. 检查其他肯定表述（如"有水肿"）
    positive_terms <- c("存在水肿", "有水肿", "可见水肿", 
                       "肺水肿", "间质性水肿", "肺泡性水肿",
                       "肺淤血", "胸腔积液伴水肿")
    if (any(sapply(positive_terms, function(term) grepl(term, output, ignore.case = TRUE)))) {
        return(list(has_edema = TRUE, evidence = "模型表述中包含肯定词汇"))
    }
    
    # 3. 检查否定表述（如"未见水肿"）
    negative_terms <- c("未见水肿", "无水肿", "未发现水肿", 
                       "肺野清晰", "肺野未见异常", "无明显水肿")
    if (any(sapply(negative_terms, function(term) grepl(term, output, ignore.case = TRUE)))) {
        return(list(has_edema = FALSE, evidence = "模型表述中包含否定词汇"))
    }
    
    # 4. 直接检查报告原文是否包含关键词（最终兜底判断）
    if (any(grepl(paste(edema_keywords, collapse = "|"), report_text, ignore.case = TRUE))) {
        return(list(has_edema = TRUE, evidence = "报告原文包含水肿相关关键词"))
    }
    
    # 5. 所有条件都不满足时，默认判断为不存在
    return(list(has_edema = FALSE, evidence = "未发现明确证据"))
}
...
analyze_single_report <- function(report_text, report_filename) {
    # 创建提示词（明确要求模型判断）
    prompt <- paste0(
        "你是专业的放射科医生，请分析以下胸片报告是否存在水肿（包括肺水肿、间质性水肿等）。\n",
        "只需根据报告内容回答，不要添加额外信息。\n",
        "输出格式必须为：\n",
        "存在水肿: 是\n",  # 或 "存在水肿: 否"
        "证据: （引用报告中支持判断的原文）\n\n",
    ...
    for (attempt in 1:max_retry) {
        tryCatch({
            ...
            if (length(output) > 0) {
                ...
                return(list(
                    success = TRUE,
                    has_edema = parsed_result$has_edema,
...
main <- function() {
    ...
    for (i in 1:total_files) {
        ...
        tryCatch({
            ...
            result_value <- ifelse(analysis_result$has_edema, 1, 0)
            ...
            if (analysis_result$success) {
                ...
                if (analysis_result$has_edema) {
                    edema_count <- edema_count + 1
                }
    ...
    cat(paste0("存在水肿的报告数: ", edema_count, " 份\n"))
    ...
    log_message(paste0("存在水肿的报告数: ", edema_count, " 份"))
