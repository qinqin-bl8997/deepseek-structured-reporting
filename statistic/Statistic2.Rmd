...
if (!"study_id" %in% colnames(huatuo_data)) {
    stop("huatuoPE.txt中未找到study_id列，请检查列名是否正确")
}
if (!"Hpleural_effusion" %in% colnames(huatuo_data)) {
    stop("huatuoPE.txt中未找到Hpleural_effusion列，请检查列名是否正确")
}

# 基于study_id进行内连接（只保留两个文件都存在的样本）
merged_data <- inner_join(
    ys_data %>% select(study_id, pleural_effusion),  # 仅保留需要的列
    huatuo_data %>% select(study_id, Hpleural_effusion),
    by = "study_id"  # 连接键
)
